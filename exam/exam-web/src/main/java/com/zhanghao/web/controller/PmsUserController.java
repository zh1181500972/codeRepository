...
public class PmsUserController extends BaseController {
	...
	public DwzParam viewPmsUserUI(Long userId) {
		try {
			PmsUser pmsUser = pmsUserService.findUserByUserNo("");
			if (pmsUser == null) {
	...
	public DwzParam addPmsUser(PmsUser pmsUser) {
		try {
			String userNo=pmsUser.getAccount();
			String userPwd=pmsUser.getAccount();
			// 表单数据校验
	...
	private String validatePmsUser(PmsUser user) {
		...
		msg += lengthValidate("真实姓名", user.getAccount(), true, 2, 15);
		msg += lengthValidate("登录名", user.getAccount(), true, 3, 50);
		...
		// 手机号码
		String mobileNo = user.getPassword();
		...
		msg += lengthValidate("描述", user.getAccount(), true, 3, 100);
		return msg;
	}
	...
	public DwzParam deleteUserStatus(Integer id) {
